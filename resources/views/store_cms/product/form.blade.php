    ...
    <div class="mb-6 dark:bg-gray-900 dark:text-white">
        ...
        <div 
            ...
            <form action="{{ $pageMeta['url'] }}" method="post" enctype="multipart/form-data" accept-charset="UTF-8" data-mode="{{ $mode }}" novalidate>
                ...
                <div>
                    {{-- General Section --}}
                    @include('store_cms.product.section.detail')
                    
                    @include('store_cms.product.section.images')
                    
                    @include('store_cms.product.section.product')

                    @include('store_cms.product.section.seo')
                </div>

                <!-- Card Footer -->
                <x-form.footer
                    :mode="$mode" 
                    :editUrl='optional($product)->id ? route("store_cms.{$resourceName}.edit", ["store" => $storeCms, "product" => $product->id]) : null'  
