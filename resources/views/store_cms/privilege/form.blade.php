<x-layout.store.app :header="$pageMeta['title']">
    ...
    <div class="mb-6 dark:bg-gray-900 dark:text-white">
        ...
        <x-form.breadcrumb 
            ...
            :indexLink="route('store_cms.' . $resourceName . '.index', [$storeCms->slug])" 
            :action="ucfirst($mode)"
            baseLink="store_cms"
            :routeParam="[$storeCms]"
        />
        ...
        <div class="w-full bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg shadow mb-6">
            <form action="{{ $pageMeta['url'] }}" method="post" enctype="multipart/form-data" accept-charset="UTF-8" data-mode="{{ $mode }}" novalidate>
                                ...
                                <thead class="bg-gray-100">
                                    <tr>
                                        <th class="px-4 py-2 border text-center">
                                            <input type="checkbox" class="check-all-table w-4 h-4" {{ $mode === 'show' ? 'disabled' : '' }}>
                                        </th>
                                        ...
                                        @if(count($children) === 1)
                                            ...
                                            <tr class="hover:bg-gray-50">
                                                <td class="px-4 py-2 border text-center">
                                                    <input type="checkbox" class="w-4 h-4" {{ $mode === 'show' ? 'disabled' : '' }}>
                                                </td>
                                                <td class="px-4 py-2 border">
                                                    {{ $child['alias'] ?? ucfirst($child['name']) }}
                                                </td>
                                                @foreach(['viewAny','view','create','update','delete'] as $perm)
                                                    ...
                                                    <td class="px-4 py-2 border text-center">
                                                        @if($action && !empty($action['id']))
                                                            <input type="checkbox" class="w-4 h-4" value="{{ $action['id'] }}" name="permissions[]" {{ in_array($action['id'], $lists['currentPermissions']) ? 'checked' : '' }} {{ $mode === 'show' ? 'disabled' : '' }}>
                                                        @else
                                        ...
                                        @else
                                            ...
                                            @foreach($children as $child)
                                                <tr class="hover:bg-gray-50">
                                                    ...
                                                    <td class="px-4 py-2 border ps-8">
                                                        {{ $child['alias'] ?? ucfirst($child['name']) }}
                                                    </td>
                                                    @foreach(['viewAny','view','create','update','delete'] as $perm)
                                                        @php
                ...
                <x-form.footer
                    :mode="$mode" 
                    :editUrl='optional($privilege)->id ? route("store_cms.{$resourceName}.edit", ["store" => $storeCms, "privilege" => $privilege->id]) : null'  
                    :backUrl='route("store_cms.{$resourceName}.index", [$storeCms->slug])' 
                />
            </form>
        </div>
    </div>
